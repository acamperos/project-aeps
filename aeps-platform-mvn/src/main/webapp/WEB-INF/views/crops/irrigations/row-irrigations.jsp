        ...
        <% if (entTypeIrrId!=3) { %>    
            <div class="btn-group">
                ...
                <% if (usrIrrDao.getPrivilegeUser(userIrr.getIdUsr(), "crop/delete")) { %>
                    <a class="btn btn-small delete_rows_dt btn-delete" title="<s:property value="getText('link.deleteirrigation.irrigation')" />" onclick="showDialogDelete(this, 'confirm_dialog_irr', '/crop/deleteIrr.action?idIrr=${idIrr}', '/crop/searchIrr.action?idCrop=${idCrop}', 'divIrr', 'divListIrr');"><i class="icon-trash"></i></a>
                <% } %>
            </div>
        <% } %>
    <% } %>
