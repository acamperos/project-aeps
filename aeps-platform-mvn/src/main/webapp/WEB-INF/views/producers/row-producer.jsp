        ...
        <td>
            <div class="btn-group">
                <% if (usrDao.getPrivilegeUser(user.getIdUsr(), "producer/modify")) { %>
                    <a class="btn btn-small" title="Editar Productor" onclick="viewForm('/aeps-plataforma-mvn/showProducer.action?action=modify&page=<%=pageNow%>', 'idPro', <s:property value ="id_producer" />, 'Editar Productor', 1050, 550)"><i class="icon-pencil"></i></a>
                <% } %>
                <% if (usrDao.getPrivilegeUser(user.getIdUsr(), "producer/delete")) { %>
                    <a class="btn btn-small delete_rows_dt" title="Borrar Productor" onclick="showDialogDelete(this, 'confirm_dialog_producer', 'deleteProducer.action?idPro=<s:property value ="id_producer" />', 'searchProducer.action?page=<%=pageNow%>', 'divProducers', '<%=divHide%>');"><i class="icon-trash"></i></a>
                <% } %>
            </div>
        </td>
    <% } %>
