    ...
    <td>
        <div class="btn-group">
            <% if (usrDao.getPrivilegeUser(user.getIdUsr(), "soil/modify")) { %>
                <a class="btn btn-small" title="Editar Rasta" onclick="viewForm('/aeps-plataforma-mvn/soil/showSoil.action?action=modify&page=<%=pageNow%>', 'idRasta', <s:property value ="id_ras" />, 'Editar Rasta', 1050, 550)"><i class="icon-pencil"></i></a>
            <% } %>
            <% if (usrDao.getPrivilegeUser(user.getIdUsr(), "soil/delete")) { %>
                <a class="btn btn-small delete_rows_dt" title="Borrar Rasta" onclick="showDialogDelete(this, 'confirm_dialog_lot', 'deleteSoil.action?idRasta=<s:property value ="id_ras" />', 'searchSoil.action?page=<%=pageNow%>', 'divRasta', '<%=divHide%>');"><i class="icon-trash"></i></a>
            <% } %>
        </div>
    </td>
<% } %>